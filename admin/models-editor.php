...
$model = ['name' => '', 'category' => 'Women', 'measurements' => '{}', 'images' => '[]', 'sequence' => 0];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (empty($_POST) && isset($_SERVER['CONTENT_LENGTH']) && $_SERVER['CONTENT_LENGTH'] > 0) {
        ...
        $sequence = isset($_POST['model_sequence']) ? (int) $_POST['model_sequence'] : 0;
        ...
        if (empty($name)) {
            ...
            if ($id > 0) {
                $stmt = $pdo->prepare("UPDATE models SET name = ?, category = ?, measurements = ?, images = ?, sequence = ? WHERE id = ?");
                $stmt->execute([$name, $category, $measurementsJson, $imagesJson, $sequence, $id]);
            } else {
                $stmt = $pdo->prepare("INSERT INTO models (name, category, measurements, images, sequence) VALUES (?, ?, ?, ?, ?)");
                $stmt->execute([$name, $category, $measurementsJson, $imagesJson, $sequence]);
            }
            header('Location: index.php?tab=models');
            exit;
        }
    ...
    <div class="max-w-5xl mx-auto">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-4">
            ...
            <div class="flex items-center gap-3">
                <a href="index.php?tab=models"
                    class="px-6 py-3 bg-white border border-gray-200 text-gray-700 rounded-2xl text-sm font-semibold hover:bg-gray-50 transition shadow-sm">Cancel</a>
            </div>
        </div>
        ...
        <form method="POST" enctype="multipart/form-data" id="modelForm" class="space-y-8">
            <div class="bg-white p-8 md:p-12 rounded-[2.5rem] shadow-sm border border-gray-100 space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                    <div>
                        <label class="block text-xs font-bold uppercase tracking-widest text-gray-400 mb-3 ml-1">Model
                            Name *</label>
                        <input type="text" name="model_name" value="<?php echo htmlspecialchars($model['name']); ?>"
                            ...
                            placeholder="e.g. Alanna" required>
                    </div>
                    <div>
                        <label
                            class="block text-xs font-bold uppercase tracking-widest text-gray-400 mb-3 ml-1">Category
                            *</label>
                        <select name="model_category"
                            ...
                            <option value="Women" <?php echo $model['category'] == 'Women' ? 'selected' : ''; ?>>Women
                            </option>
                            <option value="Men" <?php echo $model['category'] == 'Men' ? 'selected' : ''; ?>>Men</option>
                            <option value="Kids" <?php echo $model['category'] == 'Kids' ? 'selected' : ''; ?>>Kids
                            </option>
                        </select>
                    </div>
                    <div>
                        <label
                            class="block text-xs font-bold uppercase tracking-widest text-gray-400 mb-3 ml-1">Sequence
                            (Order)</label>
                        <input type="number" name="model_sequence"
                            value="<?php echo isset($model['sequence']) ? (int) $model['sequence'] : 0; ?>"
                            class="w-full bg-gray-50 border border-gray-100 p-5 rounded-2xl outline-none focus:ring-2 focus:ring-primary/20 transition-all text-sm font-semibold"
                            placeholder="e.g. 1">
                    </div>
                </div>
                ...
                <div class="pt-6 border-t border-gray-50">
                    <div class="flex justify-between items-center mb-4">
                        <label class="block text-xs font-bold uppercase tracking-widest text-gray-400 ml-1">Measurements
                            & Details</label>
                        <button type="button" onclick="addMeasurement()"
                            class="text-[10px] bg-gray-100 hover:bg-gray-200 text-gray-700 px-3 py-1 rounded-full font-bold uppercase tracking-widest transition"><i
                                class="fas fa-plus mr-1"></i> Add Field</button>
                    </div>
                ...
                <!-- Images -->
                <div class="pt-6 border-t border-gray-50">
                    <label class="block text-xs font-bold uppercase tracking-widest text-gray-400 mb-4 ml-1">Portfolio
                        Images</label>
                    ...
                    <div class="relative w-full">
                        ...
                        <div
                            class="w-full border-2 border-dashed border-gray-200 rounded-2xl p-8 flex flex-col items-center justify-center text-gray-400 bg-gray-50/50 hover:bg-gray-50 transition">
                            <i class="fas fa-cloud-upload-alt text-3xl mb-3"></i>
                            <span class="text-sm font-semibold" id="file_count_text">Click or drag multiple images to
                                upload</span>
                        </div>
                    </div>
                </div>
